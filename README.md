# Blueberry GhostLink — Hostless + Bunny fallback v9
...
- v9 fixes the previous malformed HTML issue by inserting the Wisp configuration as JavaScript inside GhostLink's existing script block instead of nesting a second `<script>` element. This prevents the browser from rendering JavaScript source as visible page text.
...
This release removes the brittle Wisp fallback-block regex. The patch now anchors to GhostLink's stable `DEFAULT_WISP` assignment, so upstream whitespace/formatting changes do not stop the Docker build.
