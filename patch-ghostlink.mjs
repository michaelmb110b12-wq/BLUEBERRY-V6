...
(function () {
    ...
    const isBunnyCdn = typeof location !== 'undefined' && location.hostname.endsWith('.b-cdn.net');
    const sameOriginWisp = window.location.protocol === 'https:' && !isBunnyCdn
    ...
    const preferred = sameOriginWisp || configured[0] || existing[0]?.url || null;
    if (preferred && !window.SITE_CONFIG.defaultWisp) {
        window.SITE_CONFIG.defaultWisp = preferred;
    }
})();
`;
  // Merge configured Blueberry/Hostless endpoints immediately before GhostLink's
  // built-in fallback check. This avoids relying on brittle whitespace or on the
  // exact shape of the fallback block.
  const mergeCode = `
            const blueberryServers = Array.isArray(window.SITE_CONFIG?.wispServers)
                ? window.SITE_CONFIG.wispServers
                : [];
...
`;
  ...
  const fallbackAnchorRe = /(\n\s*)if\s*\(!WISP_SERVERS\.length\)\s*\{/m;
  must(fallbackAnchorRe.test(html), `Wisp fallback anchor not found in ${file}`);
  html = html.replace(fallbackAnchorRe, `\n$1${mergeCode}$1if (!WISP_SERVERS.length) {`);
...
console.log(`Blueberry Hostless + Bunny v9 patch applied to ${patched} file(s).`);
